...
import { Menu, X, Home, ListTodo, Calendar, Users, MessageSquare, ThumbsUp } from 'lucide-react';
import { useUIStore } from '@/stores/useUIStore';
import { useAuthStore } from '@/stores/useAuthStore';
import { ROUTES } from '@/utils/constants';

const memberLinks = [
  { to: ROUTES.MEMBER, icon: Home, label: 'Dashboard' },
  { to: ROUTES.MEMBER_TASK_STATUS, icon: ListTodo, label: 'Task Status' },
  { to: ROUTES.MEMBER_EVENTS, icon: Calendar, label: 'Events' },
  { to: ROUTES.MEMBER_LEADER_CONTACT, icon: Users, label: 'Leader Contact' },
  { to: ROUTES.MEMBER_UPDATES, icon: MessageSquare, label: 'Updates' },
  { to: ROUTES.MEMBER_FEEDBACK, icon: ThumbsUp, label: 'Feedback' },
];
...
export function MemberSidebar() {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed top-0 left-0 w-80 h-screen bg-white border-r border-gray-200 shadow-xl z-[50] overflow-y-auto transition-transform duration-300 ease-in-out md:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 mb-8 p-4 bg-gray-50 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-lg font-semibold text-gray-900 truncate">{memberName ?? 'Member'}</h1>
              <p className="text-sm text-gray-600 truncate">Member ({clubName ?? 'Club'})</p>
            </div>
          </div>
