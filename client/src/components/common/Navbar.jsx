...
const commonLinkClass = "px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200";
const activeLinkClass = "bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/30";
...
const activeMobileLinkClass = "bg-fuchsia-500/20 text-fuchsia-400";
...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-slate-900/95 backdrop-blur-lg border-b border-slate-700/50">
      ...
      {/* Mobile menu panel */}
      <div
        className={`lg:hidden fixed inset-y-0 right-0 w-full max-w-sm bg-slate-900 border-l border-slate-700 transform transition-transform duration-300 ease-in-out z-50 shadow-2xl ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        ...
        <div className="p-6 border-t border-slate-700 bg-slate-800">
          <Link to={ROUTES.LOGIN_TYPE} className="block" onClick={() => setMobileMenuOpen(false)}>
