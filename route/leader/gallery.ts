...
import Event from '../../models/Event-gallery';
...
function isLeader(req: any, res: any, next: any){
  if (req.session?.user?.clubId) return next();
  res.status(401).send('Unauthorized');
}

// POST  /leader/events  – create event
router.post('/leader/events_gallery', isLeader, upload.array('photos',5), async (req: express.Request, res: express.Response) =>{
  try{
    ...
    const photoUrls = (req as any).files.map((f: any) => `/uploads/${f.filename}`);
...
router.get('/leader/events/page', isLeader, async (req: express.Request, res: express.Response) =>{
...
router.get('/leader/manage-events', async (req: express.Request, res: express.Response) => {
  if (!req.session?.user) {
